...
class DataTransform:
    ...
    def __call__(self, kspace, target, attrs, fname, slice, sense_maps=None, vh=None):
        ...
        coil_combined_x = (coil_combined_x - np.min(coil_combined_x)) / (np.max(coil_combined_x) - np.min(coil_combined_x))
...
class MRIDataModuleDiffusion(pl.LightningDataModule):
    """
