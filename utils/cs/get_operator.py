...
class H:
    ...
    def A(self, X):
        X_i = X.reshape(X.shape[0], X.shape[1], -1)
        for j in range(self.rep_level):
            ...
            X_i = (torch.view_as_real(
                torch.fft.fft(torch.view_as_complex(X_foo_2.permute(0, 2, 1).contiguous()), norm="ortho"))).permute(0,
                                                                                                                    2,
                                                                                                                    1).contiguous()
        return X_i.reshape(X.shape)

    def H(self, Y):
        Y_i = Y.reshape(Y.shape[0], Y.shape[1], -1)
        for j in range(self.rep_level):
            Y_foo_1 = (torch.view_as_real(
                torch.fft.ifft(torch.view_as_complex(Y_i.permute(0, 2, 1).contiguous()), norm="ortho"))).permute(0, 2,
                                                                                                                 1).contiguous()
