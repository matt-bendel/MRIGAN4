...
def _get_embed_im(multi_coil_inp, mean, std, maps):
...
if __name__ == "__main__":
    ...
    feature_extractor = vgg16(pretrained=True).eval()
    feature_extractor = WrapVGG(feature_extractor).eval().cuda()
    vgg_transforms = torch.nn.Sequential(
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            test = _get_embed_im(x, mean, std, maps)
            test_feature = feature_extractor(vgg_transforms(test))
            ...
            combined_new_im = torch.sum(new_im, dim=0).abs()
