...
class MulticoilDataset(torch.utils.data.Dataset):
    def __init__(self, root, max_val_dir, img_size=320, mask_type='s4', accel_rate=4, scan_type=None, num_vcoils=8,
                 slice_range=8, val=False, test=False):
        ''' 
        ...
        for fname in files[1:len(files)]:
        ...
        files = sorted(files[0:num_files])
        print('Loading Data')
        for fname in tqdm(sorted(files)):
            ...
            with h5py.File(fname, "r") as hf:
                ...
                if hf['kspace'].shape[-1] < 384 or hf['kspace'].shape[1] < 8 or str(
                    ...
                    self.examples += [(fname, slice) for slice in range(self.slice_range)]
...
class FastMRIDataModule(pl.LightningDataModule):
    def __init__(self, base_path, batch_size: int = 32, num_data_loader_workers: int = 2, annotated=False, **kwargs):
        """
    ...
    def setup(self, stage: str = None):
        ...
        max_val_dir_train = os.path.join(self.base_path,
                                         '{0}_{1}_{2}coils_general'.format(self.challenge, 'train', self.num_vcoils))
        max_val_dir_val = os.path.join(self.base_path,
                                       '{0}_{1}_{2}coils_general'.format(self.challenge, 'val', self.num_vcoils))
