        ...
        for i, data in enumerate(test_loader):
            ...
            for j in range(y.size(0)):
                ...
                for l in range(cfg.num_z_test):
                    ...
                    ddrm_recons[l, :, :] = ndimage.rotate(recon_object[0, :, :].cpu().numpy(), 180)
                ...
                x_coord = zoom_startx + zoom_lengthx
                y_coords = [zoom_starty, zoom_starty + zoom_lengthy]
                ...
                rect = patches.Rectangle((zoom_startx, zoom_starty), zoom_lengthx, zoom_lengthy, linewidth=1,
                                         edgecolor='r',
                                         facecolor='none')
                ...
                count = 2
                for method in keys:
                    ax = plt.subplot(gs[0:2, count:count+2])
                ...
                ax = plt.subplot(gs[0:2, count+2:count+4])
                ...
                count = 2
                for method in keys:
                    ax = plt.subplot(gs[3:5, count:count+2])
                ...
                ax = plt.subplot(gs[3:5, count+2:count+4])
                ...
                for method in keys:
                    if method != 'l1_ssim' and method != 'varnet':
                        ax = plt.subplot(gs[6:8, count:count+2])
                ...
                ax = plt.subplot(gs[6:8, count+2:count+4])
                ...
                ax.imshow(np_gt[zoom_starty:zoom_starty + zoom_lengthy, zoom_startx:zoom_startx + zoom_lengthx], cmap='gray', vmin=0, vmax=0.7 * np.max(np_gt))
                ax.set_xticklabels([])
                ax.set_yticklabels([])
                ax.set_xticks([])
                ax.set_yticks([])
                ...
                connection_path_2 = patches.ConnectionPatch([x_coord, y_coords[1]], [zoom_lengthx, 0],
                ...
                count = 2
                for method in keys:
                    ax = plt.subplot(gs[2, count:count+2])
                    ax.imshow(np_avgs[method][zoom_starty:zoom_starty + zoom_lengthy, zoom_startx:zoom_startx + zoom_lengthx], cmap='gray', vmin=0, vmax=0.7 * np.max(np_gt))
                    ax.set_xticklabels([])
                    ax.set_yticklabels([])
                    ax.set_xticks([])
                    ax.set_yticks([])

                    ax = plt.subplot(gs[5, count:count+2])
                    im = ax.imshow(2 * np.abs(np_avgs[method] - np_gt)[zoom_starty:zoom_starty + zoom_lengthy,
                                       zoom_startx:zoom_startx + zoom_lengthx], cmap='jet', vmin=0,
                                   vmax=np.max(np.abs(np_avgs['rcgan_wo_gr_w_dc'] - np_gt)))
                    ax.set_xticklabels([])
                    ax.set_yticklabels([])
                    ax.set_xticks([])
                ...
                ax = plt.subplot(gs[5, count:count+2])
                ax.imshow(3 * np.abs(langevin_avg - langevin_gt)[zoom_starty:zoom_starty + zoom_lengthy,
                              zoom_startx:zoom_startx + zoom_lengthx], cmap='jet', vmin=0,
                          vmax=np.max(np.abs(np_avgs['rcgan_wo_gr_w_dc'] - np_gt)))
                ax.set_xticklabels([])
                ax.set_yticklabels([])
                ...
                ax = plt.subplot(gs[2, count+2:count+4])
                ...
                ax = plt.subplot(gs[5, count+2:count+4])
                ax.imshow(3 * np.abs(ddrm_avg - ddrm_gt)[zoom_starty:zoom_starty + zoom_lengthy,
                              zoom_startx:zoom_startx + zoom_lengthx], cmap='jet', vmin=0,
                          vmax=np.max(np.abs(np_avgs['rcgan_wo_gr_w_dc'] - np_gt)))
                ax.set_xticklabels([])
                ax.set_yticklabels([])
                ...
                for method in keys:
                    if method != 'l1_ssim' and method != 'varnet':
                        ax = plt.subplot(gs[8, count:count+2])
                        ax.imshow(np_stds[method][zoom_starty:zoom_starty + zoom_lengthy, zoom_startx:zoom_startx + zoom_lengthx], cmap='viridis', vmin=0, vmax=np.max(np_stds['rcgan_wo_gr_w_dc']))
                        ax.set_xticklabels([])
                        ax.set_yticklabels([])
                        ax.set_xticks([])
                        ax.set_yticks([])
                        ...
                        ax = plt.subplot(gs[8, count:count+2])
                        im = ax.imshow(np.zeros((384, 384))[zoom_starty:zoom_starty + zoom_lengthy, zoom_startx:zoom_startx + zoom_lengthx], cmap='viridis', vmin=0,
                ...
                ax = plt.subplot(gs[8, count:count+2])
                ax.imshow(langevin_std[zoom_starty:zoom_starty + zoom_lengthy, zoom_startx:zoom_startx + zoom_lengthx], cmap='viridis', vmin=0, vmax=np.max(np_stds['rcgan_wo_gr_w_dc']))
                ax.set_xticklabels([])
                ax.set_yticklabels([])
                ax.set_xticks([])
                ax.set_yticks([])

                ax = plt.subplot(gs[8, count+2:count+4])
                ax.imshow(ddrm_std[zoom_starty:zoom_starty + zoom_lengthy, zoom_startx:zoom_startx + zoom_lengthx], cmap='viridis', vmin=0, vmax=np.max(np_stds['rcgan_wo_gr_w_dc']))
                ax.set_xticklabels([])
                ax.set_yticklabels([])
                ax.set_xticks([])
                ax.set_yticks([])

                plt.savefig(f'mr_figs_workshop/workshop_body_R={args.R}_rect_alt_format_{fig_count}.png', bbox_inches='tight', dpi=300)
                plt.close(fig)
                if fig_count == 1:
                    print(fig_count)
                    exit()
