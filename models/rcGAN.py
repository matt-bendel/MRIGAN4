    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 1:
            ...
            if (self.global_step - 1) % self.args.pca_reg_freq == 0 and self.current_epoch >= 25:
                ...
                x_zm = x_embed - torch.mean(gens_embed, dim=1).clone().detach()
...
class EigenGANPix(pl.LightningModule):
    def __init__(self, args, num_realizations, default_model_descriptor, exp_name, noise_type, num_gpus):
        super().__init__()
        self.args = args
        self.num_realizations = num_realizations
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 1:
            ...
            if (self.global_step - 1) % self.args.pca_reg_freq == 0 and self.current_epoch >= 0:
                gens_embed = torch.zeros(
                    size=(y.size(0), self.args.num_z_pca, 4718592),
                    device=self.device)
                for z in range(self.args.num_z_pca):
                    gens_embed[:, z, :] = self.forward(y, mask).view(y.size(0), self.args.num_z_pca, -1)
